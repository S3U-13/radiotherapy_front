...
export default function MessagePage() {
  ...
  // Navigation Folders
  const folders = [
    { id: "inbox", name: "กล่องข้อความ", icon: <Inbox size={18} />, count: 3, color: "text-blue-500" },
    { id: "starred", name: "ติดดาว", icon: <Star size={18} />, count: 0, color: "text-yellow-500" },
    { id: "sent", name: "ส่งแล้ว", icon: <Send size={18} />, count: 0, color: "text-green-500" },
    { id: "drafts", name: "ฉบับร่าง", icon: <FileText size={18} />, count: 1, color: "text-gray-500" },
    { id: "important", name: "สำคัญ", icon: <AlertCircle size={18} />, count: 0, color: "text-red-500" },
    { id: "trash", name: "ถังขยะ", icon: <Trash2 size={18} />, count: 0, color: "text-neutral-500" },
  ];
  ...
  const mockMessages = [
    {
      ...
      snippet: "เรียน บุคลากรทุกท่าน ขอความร่วมมือในการทำแบบประเมินผลการปฏิบัติงาน...",
      time: "10:45 น.",
      unread: true,
      labels: ["สำคัญ", "ประกาศ"],
    },
    {
      ...
      snippet: "สวัสดีครับ รบกวนตรวจสอบแผนการรักษาของเคสผู้ป่วยหมายเลข 445210...",
      time: "เมื่อวาน",
      unread: true,
      labels: ["ปรึกษาแพทย์"],
    },
    {
      ...
      snippet: "แจ้งปิดปรับปรุงระบบชั่วคราวในวันเสาร์ที่ 15 เมษายน ตั้งแต่เวลา 01:00 - 04:00 น.",
      time: "1 เม.ย.",
      unread: false,
      labels: ["ระบบ"],
    },
    {
      ...
      snippet: "ขอเชิญเข้าร่วมประชุมประจำเดือนในวันพุธหน้า เวลา 13:00 น. ณ ห้องประชุม 3",
      time: "28 มี.ค.",
      unread: false,
      labels: ["การประชุม"],
    },
            ...
            {folders.map((folder) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`${activeFolder === folder.id ? folder.color : ""}`}>
                    {folder.icon}
                  </div>
        ...
        <div className="h-20 border-b border-gray-100 dark:border-neutral-800/50 flex items-center justify-between px-6 lg:px-8 shrink-0">
          ...
          <div className="flex flex-1 max-w-md ml-8 justify-end">
            <Input
              ...
              classNames={{
                input: "text-sm",
                inputWrapper: "bg-gray-100/50 dark:bg-neutral-800/50 dark:hover:bg-neutral-800 border border-transparent shadow-none",
              }}
            />
          </div>
        </div>
          ...
          {mockMessages.length > 0 ? (
            <div className="flex flex-col">
              {mockMessages.map((msg) => (
                <div 
                  key={msg.id} 
                  className={`group flex items-start gap-4 p-4 lg:p-6 border-b border-gray-50 dark:border-neutral-800/30 cursor-pointer hover:bg-gray-50/80 dark:hover:bg-[#18181B] transition-all duration-200 ${
                    msg.unread ? "bg-white dark:bg-[#131317]" : "bg-gray-50/30 dark:bg-[#131317]/50"
                  }`}
                >
                  <div className="shrink-0 mt-1">
                    {msg.avatar ? (
                      <Avatar src={msg.avatar} alt={msg.sender} className="w-12 h-12 text-large" />
                    ) : (
                      <Avatar name={msg.sender.charAt(0)} className="w-12 h-12 bg-neutral-200 dark:bg-neutral-700 text-neutral-600 dark:text-neutral-300" />
                    )}
                  </div>
                  
                  <div className="flex-1 min-w-0 pr-4">
                    <div className="flex justify-between items-center mb-1">
                      <h3 className={`text-[15px] truncate ${msg.unread ? 'font-semibold text-neutral-900 dark:text-neutral-100' : 'font-medium text-neutral-700 dark:text-neutral-300'}`}>
                        {msg.sender}
                      </h3>
                      <span className={`text-xs whitespace-nowrap ml-4 ${msg.unread ? 'font-medium text-blue-600 dark:text-blue-400' : 'text-neutral-500'}`}>
                        {msg.time}
                      </span>
                    </div>
                    
                    <div className="flex items-center gap-2 mb-1.5 overflow-hidden">
                      <p className={`text-sm truncate ${msg.unread ? 'font-medium text-neutral-800 dark:text-neutral-200' : 'text-neutral-600 dark:text-neutral-400'}`}>
                        {msg.subject}
                      </p>
                    </div>
                    ...
                    <div className="mt-3 flex gap-2">
                      {msg.labels.map(label => (
                        <Chip key={label} size="sm" variant="flat" className="bg-gray-100 dark:bg-neutral-800/80 text-neutral-600 dark:text-neutral-300 h-6 text-[10px] px-1 border border-gray-200 dark:border-neutral-700">
                          {label}
                        </Chip>
                      ))}
                    </div>
                  ...
                  <div className="shrink-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <Button isIconOnly variant="light" size="sm" radius="full" className="text-neutral-400 hover:text-neutral-800 dark:hover:text-white">
                      <Star size={18} />
                    </Button>
                    <Button isIconOnly variant="light" size="sm" radius="full" className="text-neutral-400 hover:text-neutral-800 dark:hover:text-white">
